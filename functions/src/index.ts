...
function parseAssessmentPath(filePath: string): { communeId: string; periodId: string; indicatorId: string; docIndex: number; } | null {
  // Path for Criterion 1 & 4: hoso/{communeId}/evidence/{periodId}/{indicatorId}/{docIndex}/{fileName}
  const pathRegex = /^hoso\/([^\/]+)\/evidence\/([^\/]+)\/(CT1\.[1-3]|CT2\.4\.1)\/(\d+)\//;
  ...
  if (match) {
    return {
      ...
      periodId: match[2],
      indicatorId: match[3],
      docIndex: parseInt(match[4], 10),
    };
  }

  // Path for simple indicators (không có docIndex)
  const simplePathRegex = /^hoso\/([^\/]+)\/evidence\/([^\/]+)\/([^\/]+)\/[^\/]+$/;
  const simpleMatch = filePath.match(simplePathRegex);
  if (simpleMatch) {
    // Return null for simple indicators for now as they don't trigger this logic
    return null;
  }
  ...
  for (const fileUrl of filesBefore) {
    if (!filesAfter.has(fileUrl) && fileUrl.includes("firebasestorage.googleapis.com")) {
      ...
      try {
        ...
        // Update assessment status after file deletion
        ...
        if (pathInfo) {
          ...
          const transactionPromise = db.runTransaction(async (transaction) => {
            ...
            if (indicatorResult?.filesPerDocument?.[pathInfo.docIndex]) {
              const filesForDoc = indicatorResult.filesPerDocument[pathInfo.docIndex];
              if (filesForDoc.length === 0) {
                // Logic to re-evaluate the status
                const criterionDocRef = db.collection('criteria').doc(pathInfo.indicatorId.startsWith('CT1') ? 'TC01' : 'TC02');
                const criterionDoc = await transaction.get(criterionDocRef);
...
export const verifyPDFSignature = onObjectFinalized({
  ...
  try {
    ...
    const isValid = !isLate && !isEarly;

    let failureReason = "Chữ ký hợp lệ";
    let statusForLog: "valid" | "expired" | "error" = "valid";

    if (isLate) {
      failureReason = `Ký sau thời hạn (${deadline.toLocaleDateString("vi-VN")})`;
      statusForLog = "expired";
    } else if (isEarly) {
      failureReason = `Ký trước ngày ban hành (${issueDate.toLocaleDateString("vi-VN")})`;
      statusForLog = "error";
    }
    ...
    await updateAssessmentFileStatus(
      ...
      isValid ? undefined : failureReason);
  } catch (error: unknown) {
...
export const verifyCT4Signature = onObjectFinalized({
  ...
  const updateAssessmentFileStatus = async (
    ...
    try {
      await db.runTransaction(async (transaction) => {
        ...
        const indicatorResult = assessmentData[indicatorId] || { filesPerDocument: {}, status: 'pending', value: 0 };
        const filesPerDocument = indicatorResult.filesPerDocument || {};
        const fileList: { name: string, url: string, signatureStatus?: string, signatureError?: string }[] = filesPerDocument[docIndexToUpdate] || [];
        ...
        filesPerDocument[docIndexToUpdate] = fileList;
        indicatorResult.filesPerDocument = filesPerDocument;
  ...
  try {
    await updateAssessmentFileStatus(docIndex, "validating");
    ...
    const criterionDocRef = db.collection("criteria").doc("TC02");
    const criterionDoc = await criterionDocRef.get();
    if (!criterionDoc.exists) throw new Error(`Criterion config TC02 not found.`);
    const criterionData = criterionDoc.data();
    ...
    const assignmentType = criterionData?.assignmentType || "quantity";
    ...
    if (assignmentType === "specific") {
      ...
      issueDate = parse(documentConfig.issueDate, "dd/MM/yyyy", new Date());
      const issuanceDeadlineDays = documentConfig.issuanceDeadlineDays ?? 7;
    ...
    if (isNaN(issueDate.getTime())) {
      throw new Error(`Invalid issue date format. Use DD/MM/YYYY.`);
    }
    ...
    const reason = isValid
      ? undefined
      : `Ký sau thời hạn ${criterionData?.documents?.[docIndex]?.issuanceDeadlineDays || 7} ngày làm việc (Hạn chót: ${deadline.toLocaleDateString("vi-VN")})`;

    await saveCheckResult(
      isValid ? "valid" : "expired",
      reason || (isValid ? "Chữ ký hợp lệ" : "Ký sau thời hạn"),
      signingTime, deadline, firstSignature.name
    );
    ...
    await saveCheckResult("error", userFriendlyMessage);
    await updateAssessmentFileStatus(docIndex, "error", userFriendlyMessage);
    return null;
  }
  return null;
});
