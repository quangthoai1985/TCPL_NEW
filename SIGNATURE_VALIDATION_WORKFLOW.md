...
## Quy trình hoạt động cho Nội dung 1, Chỉ tiêu 4, Tiêu chí 2 (Ban hành Kế hoạch PBGDPL - CT2.4.1)
...
1.  **Nhập liệu (Phía Client):**
    ...
    *   Tiếp theo, người dùng tải lên tệp PDF là "Kế hoạch PBGDPL" của xã mình. Tệp này được tải lên Storage với một đường dẫn đặc biệt:
        `hoso/{communeId}/evidence/{periodId}/CT2.4.1/{docIndex}/{fileName}` (Lưu ý: `docIndex` thường là 0 cho trường hợp này).
...
3.  **So sánh và Cập nhật:**
    ...
    *   Nó cập nhật trạng thái `signatureStatus` ("valid" hoặc "invalid") vào thông tin của chính tệp tin đó trong `assessmentData`.
