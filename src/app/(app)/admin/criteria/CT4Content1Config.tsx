...
function CT4Content1Config({ criterion, onSave }: { criterion: Criterion, onSave: (criterion: Partial<Criterion>) => void }) {
    ...
    const [formData, setFormData] = React.useState<{
        ...
        documents: { name: string; issueDate: string; excerpt: string; issuanceDeadlineDays: number }[];
    }>({
        assignmentType: criterion.assignmentType || 'specific',
        assignedDocumentsCount: criterion.assignedDocumentsCount || 0,
    ...
    const handleSave = () => {
        ...
        if (formData.assignmentType === 'specific') {
            updatedCriterion.documents = formData.documents || [];
        } else {
            // Ensure the documents field is not part of the object if not specific
            delete updatedCriterion.documents;
        }
                            ...
                            {formData.documents?.map((doc, index) => (
                                <div key={index} className="grid grid-cols-1 md:grid-cols-10 gap-x-4 gap-y-3 items-start p-3 border rounded-md bg-orange-50 border-orange-300">
                                    ...
                                    <div className="md:col-span-9 grid gap-1.5">
                                        <Label htmlFor={`doc-name-${index}-ct4`} className="text-xs font-semibold text-destructive">Tên Kế hoạch PBGDPL</Label>
                                        <Input id={`doc-name-${index}-ct4`} value={doc.name} onChange={(e) => handleDocumentChange(index, 'name', e.target.value)} placeholder={`Ví dụ: Kế hoạch PBGDPL năm 2025...`} />
                                    </div>
                                    ...
                                    <div className="md:col-span-4 grid gap-1.5">
                                        <Label htmlFor={`doc-deadline-${index}-ct4`} className="text-xs font-semibold text-destructive">Thời hạn ban hành (ngày làm việc)</Label>
