...
function RevisionDialog({ indicator, data, onSave, onCancel, criteria, uploadFile, communeId, periodId }: { indicator: Indicator | SubIndicator, data: IndicatorResult, onSave: (id: string, newData: IndicatorResult) => void, onCancel: () => void, criteria: Criterion[], uploadFile: (file: File, communeId: string, periodId: string, indicatorId: string) => Promise<string>, communeId: string, periodId: string }) {
    ...
    try {
      ...
      for (const file of files) {
        if (file instanceof File) {
          try {
            const url = await uploadFile(file, communeId, periodId, indicator.id);
            uploadedFiles.push({ name: file.name, url });
          } catch (error) {
            console.error(`Upload error for ${file.name}`, error);
            toast({ variant: 'destructive', title: 'Lỗi tải lên', description: `Không thể tải lên tệp: ${file.name}` });
      ...
      onSave(indicator.id, { ...data, value, isTasked, communeNote: note, files: uploadedFiles });
      onCancel();
    } catch (error) {
      console.error("Save revision error", error);
      toast({ variant: 'destructive', title: 'Lỗi lưu', description: 'Đã có lỗi xảy ra khi lưu.' });
    } finally {
      setIsUploading(false);
    }
  };
  ...
  return (
    <DialogContent className="max-w-2xl">
      ...
      <DialogFooter>
        <Button variant="outline" onClick={onCancel} disabled={isUploading}>Hủy</Button>
...
export default function AssessmentDetailPage() {
  ...
  const { role, units, assessments, updateAssessments, criteria, currentUser, uploadFile } = useData();
  const { toast } = useToast();
                        ...
                        {index === 0 ? (() => {
                                      ...
                                      return (
                                        <div key={indicator.id} className={blockClasses}>
                                          ...
                                          <div className="grid gap-1">
                                            ...
                                            <p className="text-sm text-muted-foreground whitespace-pre-line">{result.note || "Không có giải trình."}</p>
                                          </div>
                          ...
                          criterion.indicators.map(indicator => {
                            const renderIndicatorContent = (ind: typeof indicator | (typeof indicator.subIndicators)[0], isSub: boolean) => {
                              ...
                              const isProgressIndicator = ind.id === 'CT2.2' || ind.id === 'CT1.2' || ind.id === 'CT1.3';
                              let assignedCount = 0;
                              let progressPercentage = 0;
                              let sourceLabel = '';

                              if (isProgressIndicator && ind.parentCriterionId === 'TC01') {
                                const tc01Criterion = criteria.find(c => c.id === 'TC01');

                                if (ind.id === 'CT2.2') {
                                  // ==== CT2.2: LOGIC ƯU TIÊN 1→2→3 ====
                                  const ct1_1_result = assessment?.assessmentData?.['CT1.1'];
                                  const communeDefinedDocuments = ct1_1_result?.communeDefinedDocuments;
                                  ...
                                  // ƯU TIÊN 2: Admin giao số lượng
                                    ...
                                    sourceLabel = 'Admin giao số lượng';
                                  }
                                  // ƯU TIÊN 3: Xã tự khai báo
                                  else {
                                    const ct1_1_value = ct1_1_result?.value;
                                    if (ct1_1_value && Number(ct1_1_value) > 0) {
                                      assignedCount = Number(ct1_1_value);
                                      sourceLabel = 'Xã tự khai báo';
                                  ...
                                  // ==== CT1.2, CT1.3: Lấy từ VALUE CT1.1 ====
                                  const ct1_1_result = assessment?.assessmentData?.['CT1.1'];
                                  assignedCount = Number(ct1_1_result?.value) || 0;
                                  sourceLabel = 'Số văn bản xã đã ban hành (CT1.1)';
                                }

                                progressPercentage = assignedCount > 0
                                  ? Math.round((Number(result.value) || 0) / assignedCount * 100)
                                  : 0;
                              }
                              ...
                              return (
                                <div key={ind.id} className={blockClasses}>
                                  ...
                                  {isProgressIndicator && assignedCount > 0 && (
                                    <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                                      <div className="flex justify-between items-center mb-2">
                                        <Label className="text-sm font-medium">
                                          {ind.id === 'CT2.2'
                                            ? 'Tiến độ công khai văn bản'
                                            : 'Tiến độ hoàn thành'}
                                        </Label>
                                        <span className="text-sm font-bold">{progressPercentage}%</span>
                                      </div>
                                      <Progress value={Math.min(progressPercentage, 100)} indicatorClassName={progressPercentage >= 100 ? 'bg-green-500' : 'bg-yellow-500'} className="h-2" />
                                      <p className="text-xs text-muted-foreground mt-2">
                                        Đã hoàn thành <strong>{result.value || 0}</strong> / <strong>{assignedCount}</strong>
                                        <span className="ml-1 text-blue-700">({sourceLabel})</span>
                                      </p>
                                    </div>
                                  )}

                                  {ind.templateFiles && ind.templateFiles.length > 0 && (
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-3 bg-blue-50 border border-blue-200 rounded">
                                      <div className="md:col-span-1 font-medium text-blue-900">File mẫu đã giao:</div>
                                      <div className="md:col-span-2 space-y-1">
                                        {ind.templateFiles.map((file, idx) => (
                                  ...
                                  <>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                      ...
                                      <p className="md:col-span-2 text-sm whitespace-pre-line">{result.note || "Không có giải trình."}</p>
                                    </div>
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <Dialog open={!!revisingIndicator} onOpenChange={(open) => !open && setRevisingIndicator(null)}>
        {revisingIndicator && (
          <RevisionDialog
            ...
            periodId={assessment.assessmentPeriodId}
          />
        )}
      </Dialog>
